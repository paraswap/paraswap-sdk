...
export class Curve extends Adapter {
  static getDexData(optimalRate: OptimalRate, name: string): CurveDexData {
    const { srcAmount, data: { exchange, i, j, underlyingSwap } } = optimalRate;
    return {
      name,
      srcAmount,
      destAmount: '1',
  ...
  private _swap(srcToken: Address, destToken: Address, data: CurveDexData) {
    ...
    const curveContract = new this.web3Provider.eth.Contract(CURVE_ABI, exchange);

    const defaultArgs = [i, j, data.srcAmount, data.minConversionRate];

    const swapMethod = underlyingSwap ? curveContract.methods.exchange_underlying : curveContract.methods.exchange;
  ...
  async buildSwap(srcToken: Address, destToken: Address, data: CurveDexData): Promise<DexParams> {
