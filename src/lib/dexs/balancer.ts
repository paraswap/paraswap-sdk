...
export class Balancer extends Adapter {
  ...
  private getFillMethod(
    srcToken: Address,
    destToken: Address,
    data: BalancerDEXData,
  ) {
    ...
    if (this.isETHAddress(srcToken)) {
      return contract.methods
        .batchEthInSwapExactIn(data.swaps, destToken, data.destAmount)
        .encodeABI();
  ...
  async buildSwap(
    ...
    return this.swap(srcToken, destToken, data, swapData, data.exchangeProxy);
  }
}
