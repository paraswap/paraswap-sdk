...
export class Bancor extends Adapter {
  ...
  private _swap(srcToken: Address, destToken: Address, data: BancorDEXData) {
    ...
    const swapData = contract.methods
      .convert2(data.path, data.srcAmount, data.minDestToken, NULL_ADDRESS, '0')
      .encodeABI();

    return super.swap(srcToken, destToken, data, swapData, bancorNetwork);
  }

  async buildSwap(
    srcToken: Address,
    destToken: Address,
    data: Required<BancorDEXData>,
  ): Promise<DexParams> {
